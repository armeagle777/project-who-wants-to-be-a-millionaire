...
function countdown(){
    ...
    gifTimer=setTimeout(function(){$("#countdown").css("display","none");},30000);
     ...
     $(".content").each(function(){
         if($(this).text()==questObj["answer"]["tru"][i]){
             ...
             setInterval(function(){
                 ...
                 setTimeout(function(){$(".correctAns").css("background","#000");},300);
                 
             },600);
             
         }
         setTimeout(function(){
             $("body").html("<img src='../images/loser.gif'>");
         },3000);
     })
 }           

var questObj ={
    "question":["Նշվածներից ո ՞րն է հայ հնագույն վիպերգերից մեկը:","  Ինչպե՞ս են անվանել հայ հին վիպերգերը","Ո՞վ է Արտաշեսի նախատիպը «Արտաշես և Արտավազդ» վիպերգում:","Արտաշես անունը ծագել է պարսկական Արտաքսերքսես թագավորի անունից, որը ստուգաբանվում է`","Արտավազդ անունը ծագել է պահլ. Artavazd անունից, որ հայերի մեջ տարածված էր մ.թ.ա. IV դարում: Ստուգաբանվում է`",".Մասիսը Արարատ լեռան անուններից մեկն է: Ստուգաբանվում է`","Ովքե՞ր են ալանները:","Նշվածներից ո ՞րն է հայ հնագույն վիպերգերից մեկը:","  Ինչպե՞ս են անվանել հայ հին վիպերգերը","Ո՞վ է Արտաշեսի նախատիպը «Արտաշես և Արտավազդ» վիպերգում:","Արտաշես անունը ծագել է պարսկական Արտաքսերքսես թագավորի անունից, որը ստուգաբանվում է`","Արտավազդ անունը ծագել է պահլ. Artavazd անունից, որ հայերի մեջ տարածված էր մ.թ.ա. IV դարում: Ստուգաբանվում է`",".Մասիսը Արարատ լեռան անուններից մեկն է: Ստուգաբանվում է`","Ովքե՞ր են ալանները:"], 
    "answer":{
        "tru":["«Անահիտ»","Գողթան երգեր","Արտաշես Բ"," «լույս»","«արդարություն»","«սար»","աբխազների նախնիները","«Անահիտ»","Գողթան երգեր","Արտաշես Բ"," «լույս»","«արդարություն»","«սար»","աբխազների նախնիները"],
        "false":[
            ["«Հայկ Դյուցազն»","«Տորք Անգեղ»","«Արտաշես և Արտավազդ»"],[" Գուսանական երգեր","Ֆիդայական երգեր","Հայրենասիրական երգեր"],["Արտաշես Ա","Արտաշես Գ","Տիգրան Մեծ"],["«բարություն»","«արդարություն»","«օրհնանք»"],["«տևական»","«չարություն»","«անեծք»"],["«մայր սար»","«բարձր սար»","«մեծ գագաթ, լեռ»"],[" չեչենների նախնիները"," օսերի նախնիները","ասորիների նախնիները"],["«Հայկ Դյուցազն»","«Տորք Անգեղ»","«Արտաշես և Արտավազդ»"],[" Գուսանական երգեր","Ֆիդայական երգեր","Հայրենասիրական երգեր"],["Արտաշես Ա","Արտաշես Գ","Տիգրան Մեծ"],["«բարություն»","«արդարություն»","«օրհնանք»"],["«տևական»","«չարություն»","«անեծք»"],["«մայր սար»","«բարձր սար»","«մեծ գագաթ, լեռ»"],[" չեչենների նախնիները"," օսերի նախնիները","ասորիների նախնիները"]
        ]
    }
};
function begin(){
...
//    document.getElementById("video").play();
    ...
    $("#head_cover").css("display","block");
                ...
                if(callHelpButton == true && liPlay == true){
                    $("#money_content ol li").eq(13-i+1).removeClass("active");
                $("#money_content ol li").eq(13-i).addClass("active");
                }else if(callHelpButton == false && liPlay == true){
                    $("#money_content ol li").eq(13-i+2).removeClass("active");
                $("#money_content ol li").eq(13-i+1).addClass("active");
                }
                
                document.getElementById("bgsound").play();
                
                var questionList = [questObj["answer"]["tru"][i], questObj["answer"]["false"][i][0], questObj["answer"]["false"][i][1], questObj["answer"]["false"][i][2]];
               ...
               document.getElementById("sectora").style.backgroundColor="rgba(0,0,0,0)";                document.getElementById("sectorb").style.backgroundColor="rgba(0,0,0,0)";                document.getElementById("sectorc").style.backgroundColor="rgba(0,0,0,0)";                document.getElementById("sectord").style.backgroundColor="rgba(0,0,0,0)";
                bg.play();
...
function fiftyFunc(){
    ...
    if(q== 0 && document.getElementsByClassName("ans")[aaa].childNodes[1].childNodes[0].textContent==questObj["answer"]["tru"][i]){
        var aaa=arrayChooseOne[1];
    }
    if(q== 1 && document.getElementsByClassName("ans")[aaa].childNodes[1].childNodes[0].textContent==questObj["answer"]["tru"][i]){
        var aaa=arrayChooseOne[0];
    }
    if(w == 0 && document.getElementsByClassName("ans")[bbb].childNodes[1].childNodes[0].textContent==questObj["answer"]["tru"][i]){
        var bbb=arrayChooseTwo[1];
    }
    if(w== 1 && document.getElementsByClassName("ans")[bbb].childNodes[1].childNodes[0].textContent==questObj["answer"]["tru"][i]){
    ...
    document.getElementById("fifty").style.background="url(../images/fiftyx.png)";
...
function hallFunc(){
    ...
    $(".bar").eq(indexOfTrue).css("height",aPercent +"%");
    $(".bar").eq(indexOfTrue).html(aPercent +"%");
    for(u=0;u<4;u++){
        if(u != indexOfTrue){
            falseArray.push(u);
        }
     ...
     for(var p = 0;p<3;p++){
         $(".bar").eq(falseArray[p]).css("height",percentArray[p] +"%");
         $(".bar").eq(falseArray[p]).html(percentArray[p] +"%");
     }
...
function callFunc(){
    ...
    document.getElementById("call").style.background="url(../images/chancex.png)";
